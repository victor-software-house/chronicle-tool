...
* `Sources/Chronicle/Core/Audio/CoreAudioTapSource.swift` — CoreAudio process tap source; creates a `CATapDescription`, private aggregate device, IOProc, default-output listener, and idle-tolerant no-buffer warning.
...
`mic` and `sysaudio` accept `--diarize`. Current commands remain one-source-per-transcriber: `mic` opens `MicAudioSource`; `sysaudio` opens `CoreAudioTapSource`. Do not mix microphone and system buffers into one raw `AnalyzerInput` sequence unless the merged stream carries explicit source metadata first; `AnalyzerInput(buffer:)` itself does not preserve source identity.

When diarization is set, the same buffers fed into each source's analyzer are also fed into FluidAudio's `SortformerDiarizer`. Speaker IDs are merged into each final by aligning audio ranges.

**Acceptance criteria:**

```gherkin
Given chronicle mic --locale pt-BR --diarize is running
And two speakers take turns speaking for 60 seconds
When the daemon emits finals
Then each final in finals.md is prefixed with the speakerId, e.g. "[S0] olá" / "[S1] tudo bem"
And the JSONL trace event objects carry a "speakerId" field
And the speaker count converges to 2 within the first ~20 seconds
```

Speaker labels are 0-indexed (`S0`, `S1`, ...) to match FluidAudio's native `speakerIndex` so downstream tools do not need to re-map between the on-disk trace and the model's segment metadata. The original PRD draft used `[S1]/[S2]`; the implementation uses `[S0]/[S1]`.

First-label convergence: on `SortformerConfig.default` the model fills a 188-frame speaker cache (\~15 s @ 80 ms/frame) before emitting confident segments. In production runs against ElevenLabs TTS at \~62 s the first `speakerId` hit appeared at \~18 s of audio; with two human voices on real speech, convergence typically lands inside the original \~10 s budget.

**Implemented receipts (2026-05-17):**

* `Sources/Chronicle/Core/Audio/BufferMulticast.swift` ships the generic `BufferMulticast<Element>` with `subscribe()` / `yield(_:)` / `finish()`. Per-subscriber queue uses `AsyncStream(bufferingPolicy: .bufferingNewest(N))` so a slow diarizer drops stale audio and stays near real-time, rather than replaying old buffers while live audio is dropped. Subscriber-list mutation is guarded by a brief `NSLock`; this is not the lock-free SPMC ring envisioned by ADR-0001 §Consequences but is functionally equivalent at the live buffer rate (\~100 buffers/s) and well within the latency budget. A future revision may swap to a true hand-rolled SPMC ring if measurements demand it.
* `Sources/Chronicle/Core/Diarize/StreamingDiarizer.swift` ships the `StreamingDiarizing` protocol, the pure `DiarizationTimelineLookup` value type (midpoint-inclusion lookup over `[DiarizationSegment]`), the `StreamingDiarizerBackend` protocol with a production `SortformerBackend` wrapping FluidAudio's `SortformerDiarizer`, and the `SortformerStreamingDiarizer` actor that composes a `PCMFloatConverter` with the backend, throttles `process()` every \~1 s, and rebuilds the timeline-lookup snapshot from `finalizedSegments + tentativeSegments` after each update / `finalize()`. `prepare()` explicitly prewarms Sortformer; model loading is reentrancy-safe so `prepare()` and first `ingest(_:)` share one backend load if they race. The backend protocol is an addition not listed in the original PRD file breakdown; it exists so the streaming flow is unit-testable without loading CoreML.
* `Sources/Chronicle/Core/Audio/PCMFloatConverter.swift` (also new) provides PCM-to-16-kHz-mono-Float conversion with dedicated Int16 / Float32 fast paths and an `AVAudioConverter` slow path. This is the hot path for mic and sysaudio (both deliver SpeechAnalyzer's preferred format, 16 kHz mono Int16, directly) and exists because the original inline `AVAudioConverter` callback path silently returned `nil` for every back-to-back same-rate buffer in production.
...
* `Tests/ChronicleTests/Audio/PCMFloatConverterTests.swift` covers Int16 16 kHz mono fast path, Float32 16 kHz mono fast path, repeated calls without state corruption, empty buffer rejection, and resampling slow paths for Float32 48 kHz and Int16 48 kHz inputs (7 tests).
...
* `Tests/ChronicleTests/Diarize/StreamingDiarizerWiringTests.swift` exercises the actor + backend wiring with a stub backend: `load()` exactly once on first ingest, `addAudio` per buffer with correct sample count, `process()` throttle every `processEverySamples`, `absorb` merging into the timeline lookup, `finish()` invoking `finalize()`, `finish()` skipping when no audio ingested, and lookup query / hit counters (7 tests).
...
* Live smoke against an ElevenLabs Sarah US-female + George UK-male fixture (\~62 s) routed through the laptop speakers: `chronicle sysaudio --diarize` produced 11 finals (6 `[S0]` + 5 `[S1]`) with 2 speakers in the live lookup and 109/177 lookup hits; `chronicle mic --diarize` produced 13 finals with consistent `[S0]` Sarah / `[S1]` George labels, 2 speakers, 140/217 lookup hits; `chronicle merge trace.mic.jsonl trace.sys.jsonl` interleaved both runs chronologically with `[source] (speaker, locale) text` lines.
* `Sources/Chronicle/Core/Speech/TranscriptionLatencyMonitor.swift` records speech-end-to-transcript receipt latency as `wallclockOffsetMs - audioRange.endSeconds * 1000`. `JSONLTraceSink` stores this as optional `transcriptionLatencyMs`; live commands emit `[mic.latency ...]` / `[sysaudio.latency ...]` diagnostics immediately during capture with rolling average, p95, max, final count, and speaker-known/unknown counts.
* Diagnostic stderr (`[diarize.diag tick]` every \~5 s, plus a `final` line on shutdown) reports ingest count, total audio seconds, process calls, non-nil updates, finalized/tentative segment counts, lookup queries and hits. Not specified by the original PRD; added during the FR-4 bring-up debug session and retained as an operator observability surface.
...
* `Sources/Chronicle/Core/Diarize/StreamingDiarizer.swift` — streaming Sortformer-backed diarizer plus the pure `DiarizationTimelineLookup` value type and reentrancy-safe `prepare()` prewarm path.
* `Sources/Chronicle/Core/Speech/TranscriptionLatencyMonitor.swift` — rolling speech-end-to-transcript latency metrics and stderr diagnostics.
...
* `Tests/ChronicleTests/Speech/TranscriptionLatencyMonitorTests.swift` — latency calculation, rolling stats, high-latency warnings, and missing-range behavior.
* `Tests/ChronicleTests/Sinks/JSONLTraceSinkTests.swift` — `didReceiveResult(speakerId:)` and `transcriptionLatencyMs` propagation.
...
| **R-A2: system audio capture permission / backend drift**                                                                       | Medium   | Medium                        | ADR-0004 replaced SCStream with `CoreAudioTapSource` because ad-hoc Tahoe builds require CoreAudio process taps for reliable system audio. CLI now uses CoreAudio tap, validates tap ASBD before conversion, requires `.app` bundle TCC identity, and treats no-buffer startup as idle output by warning while capture stays running.      |
