...
| **P11** | ALAC production audio sink | FR-1 | ✔ **done** (amended) | `Core/Sinks/ExtAudioFileALACSink` (replaced `AVAudioFileALACSink` 2026-06-03 after Tahoe pakt regression — see ADR-0002 amendment) + `Core/Sinks/RollingPCMScratchSink`. `ExtAudioFileDispose` reliably writes pakt. Default `--audio-format` is composite ALAC + scratch; `--rotate-audio` segments ALAC/WAV/Opus by audio duration. |
...
| **P14** | Audio recording reliability | — | ✔ **done** | `ExtAudioFileALACSink` replaces broken `AVAudioFileALACSink`; `chronicle repair-alac` recovers broken CAFs; `chronicle gc` cleans empty sessions; write-time integrity probe; non-blocking ChronicleApp stop button. Governed by `.kiro/specs/audio-recording-reliability/`. |
