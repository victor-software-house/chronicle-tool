...
#   3. `chronicle daemon status` returns a JSON-RPC response containing the source +
#      lifecycle.
#   4. `chronicle daemon mark <label>` round-trips through the RPC layer.
#   5. `chronicle daemon clip --last-seconds 1` returns either a success result or a
#      structured error (range_unavailable / accepted placeholder).
#   6. `chronicle daemon config --diarize true` returns a JSON-RPC response.
#   7. `chronicle daemon stop` returns a JSON-RPC response and daemon shuts down on
...
# 4. mark RPC.
MARK_JSON="$("$BIN" daemon mark --source "$SRC" --client-req-id mark-smoke smoke-marker)"
...
CLIP_JSON="$("$BIN" daemon clip --source "$SRC" --last-seconds 1 --output "$CLIP_PATH" --client-req-id clip-smoke)"
echo "$CLIP_JSON" | grep -q '"jsonrpc"' || {
  echo "[smoke-daemon-integration] clip RPC missing envelope: $CLIP_JSON"; exit 1;
}

# 6. config RPC.
CONFIG_JSON="$("$BIN" daemon config --source "$SRC" --diarize true --client-req-id config-smoke)"
echo "$CONFIG_JSON" | grep -q '"jsonrpc"' || {
  echo "[smoke-daemon-integration] config RPC missing envelope: $CONFIG_JSON"; exit 1;
}

# 7. stop RPC + shutdown.
STOP_JSON="$("$BIN" daemon stop --source "$SRC" --client-req-id stop-smoke)"
